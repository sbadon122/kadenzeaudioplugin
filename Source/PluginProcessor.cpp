...
void KadenzeAudioPluginAudioProcessor::processBlock (AudioBuffer<float>& buffer, MidiBuffer& midiMessages)
{
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
        ...
        float modParam = *parameters.getRawParameterValue(KAPParameterID[kParameter_ModulationDepth]);
        float delayTimeParam = *parameters.getRawParameterValue(KAPParameterID[kParameter_DelayTime]);
        float delayFeedbackParam = *parameters.getRawParameterValue(KAPParameterID[kParameter_DelayFeedback]);
        float delayWetDryParam = *parameters.getRawParameterValue(KAPParameterID[kParameter_DelayWetDry]);
        float delayTypeParam = *parameters.getRawParameterValue(KAPParameterID[kParameter_DelayType]);
        
        mInputGain[channel]->process(channelData, inputGain, channelData, buffer.getNumSamples());
        
        float rate = channel==1 ? 0: getParameter(kParameter_ModulationRate);
        mLfo[channel] -> process(rate, modParam, buffer.getNumSamples());
        mDelay[channel]->process(channelData, delayTimeParam, delayFeedbackParam, delayWetDryParam,delayTypeParam,mLfo[channel]->getBuffer(), channelData, buffer.getNumSamples());
    
        mOutputGain[channel]->process(channelData, outputGain, channelData, buffer.getNumSamples());
...
void KadenzeAudioPluginAudioProcessor::initializeParameters()
{
    ...
    {
        parameters.createAndAddParameter(KAPParameterID[i],KAPParameterID[i], KAPParameterID[i], NormalisableRange<float>(0, 1.0f),KAPParameterDefaultValue[i],nullptr,nullptr );
